...
def submit():
    ...
    cursor.execute("""INSERT INTO "Reportes" ("FECHA", "OBSERVACIONES", "ID_OBJETO", "ID_USUARIO", "ID_REPORTE") VALUES (%s,%s,%s,%s,%s)""", (fecha, comentario,id_objeto, identificacion, id_reporte ))
    cursor.close()
    bd.commit()
    bd.close()
    session["img"]=ruta
    return jsonify({"mensaje": "Recibido correctamente", "ruta":ruta}), 400
